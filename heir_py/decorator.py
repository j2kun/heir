...
from heir_py.pipeline import run_compiler, CompilationResult
...
def compile(
    ...
    Args:
        backend: a string indicating the backend to use. Options:
          'openfhe' (default).
        backend_config: a config object to control system-specific
          paths for the backend in question.
    """

    def decorator(func):
        compilation_result = run_compiler(
            func, openfhe_config=backend_config or openfhe_config.from_os_env()
        )
        if backend == "openfhe":
