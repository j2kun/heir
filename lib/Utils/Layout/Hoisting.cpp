...
FailureOr<presburger::IntegerRelation> pushSliceLayoutThroughInsertSlice(
  ...
  // Compute the product of sizes for unit dimensions to determine the upper
  // bound for the new range variable.
  unsigned product = 1;
  for (auto dim : unitDims) {
  ...
  unsigned product2 = 1;
  for (auto dim : unitDims) {
    newRangeCoeffs[dim] = product2;
    product2 *= resultShape[dim];
  }
