...
struct ConvertAffineForOp : public OpConversionPattern<affine::AffineForOp> {
  ConvertAffineForOp(mlir::MLIRContext *context)
      : OpConversionPattern<affine::AffineForOp>(context) {}

  using OpConversionPattern::OpConversionPattern;

  LogicalResult matchAndRewrite(
      affine::AffineForOp op, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    auto newLoop = rewriter.create<affine::AffineForOp>(
        op.getLoc(), adaptor.getLowerBoundOperands(),
    ...
    for (BlockArgument arg : op.getBody()->getArguments()) {
      mp.map(arg, newLoop.getBody()->getArgument(arg.getArgNumber()));
    }
...
struct ConvertAffineYieldOp
    : public OpConversionPattern<affine::AffineYieldOp> {
  ConvertAffineYieldOp(mlir::MLIRContext *context)
      : OpConversionPattern<affine::AffineYieldOp>(context) {}

  using OpConversionPattern::OpConversionPattern;

  LogicalResult matchAndRewrite(
      affine::AffineYieldOp op, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
  ...
  void runOnOperation() override {
    ...
    patterns
        .add<SecretGenericOpLUTConversion,
             ...
             ConvertSecretConcealOp, ConvertAffineForOp, ConvertAffineYieldOp>(
