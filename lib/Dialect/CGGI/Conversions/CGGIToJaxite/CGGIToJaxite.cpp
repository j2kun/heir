...
class CGGIToJaxite : public impl::CGGIToJaxiteBase<CGGIToJaxite> {
  void runOnOperation() override {
    ...
    op->walk([&](lwe::EncodeOp encodeOp) {
      // Check if this EncodeOp has any TrivialEncrypt users
      bool hasTrivalEncryptUser = false;
      for (auto user : encodeOp.getResult().getUsers()) {
        if (isa<lwe::TrivialEncryptOp>(user)) {
          hasTrivalEncryptUser = true;
          break;
        }
      }
      if (!hasTrivalEncryptUser) {
        encodeOpsToErase.push_back(encodeOp);
      } else {
        // This shouldn't happen - all TrivialEncrypt ops should have been
        // converted
        encodeOp.emitError()
